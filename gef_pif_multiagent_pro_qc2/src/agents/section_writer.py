...
def build_messages(spec: SectionSpec, context: Dict[str, Any], example_override: str | None = None) -> List[ChatMessage]:
    ...
    directive = dedent(
        f"""
        Draft this section for {country} in paragraph form (2–5 concise paragraphs, no bullets/tables).
        ...
        - Return ONLY a JSON object with a single key "body" whose value is the paragraphs string.
        - STRICT RFC-8259 JSON; no code fences, no trailing commas, double-quoted strings.
        """
    ).strip()
...
async def draft_section(client: OpenAIClient, spec: SectionSpec, context: Dict[str, Any], example_override: str | None = None) -> str:
    messages = build_messages(spec, context, example_override=example_override)
