...
def check_cbit_projects(country_name, csv_path='projects.csv'):
    """
...
def download_file(url, output_folder='CBIT'):
    """
    Download a file from a URL and save it to the specified folder.
    Returns the path to the downloaded file or None if failed.
    """
    try:
        # Get script directory and create absolute path for output folder
        script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    if has_cbit:
        ...
        if user_input:
            ...
            if downloaded_file:
                ...
                print(f"File has been added and saved to the CBIT folder.")
            else:
